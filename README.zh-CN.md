...
安装 shell integration 后，`avm use` 会修改当前 shell，让 `CODEX_HOME`、`CLAUDE_CONFIG_DIR`、`OPENCODE_CONFIG_DIR` 等环境变量指向 AVM 管理的 runtime homes。
...
Memory 暂时不进入主路径。当前 preview 有 `avm memory import --dry-run`，但在 Agent / Environment / Package 的配置管理模型稳定之前，不应该让用户先理解 memory。
...
| 模块 | 当前已有 | 缺口 |
| --- | --- | --- |
| Agent | `create`、`list`、`show` | 缺 edit/delete/rename/clone，以及安全的 create-only 语义 |
