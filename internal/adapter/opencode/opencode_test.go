...
func TestImportReadsOpenCodeAgentFiles(t *testing.T) {
	projectRoot := t.TempDir()
	agentsDir := filepath.Join(projectRoot, ".opencode", "agents")
	...
	result, err := opencode.New(opencode.WithProjectRoot(projectRoot), opencode.WithConfigDir(filepath.Join(projectRoot, "empty-home"))).Import(context.Background())
	if err != nil {
