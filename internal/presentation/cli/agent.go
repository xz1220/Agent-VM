...
Conflicts: pass --on-conflict {overwrite|cancel|rename|skip} — default behavior
	...
	cmd.Flags().StringSliceVar(&f.skills, "skill", nil, "capability ID or unique name for a skill (repeatable)")
	cmd.Flags().StringSliceVar(&f.mcps, "mcp", nil, "capability ID or unique name for an MCP server (repeatable)")
...
Tip: read current state with  avm agent show <name> --json  before editing.`,
	...
	cmd.Flags().StringSliceVar(&f.skills, "skill", nil, "replace skills list (capability IDs or unique names)")
	cmd.Flags().StringSliceVar(&f.mcps, "mcp", nil, "replace MCP list (capability IDs or unique names)")
