...
2. 根据 resolved active 和当前 runtime 的 Agent Profile 生成 render plan。
3. 写入 runtime managed paths。
4. 返回字段级 mapping status。
5. 提供冲突检测和备份所需的 managed paths。
...
Adapter 不再提供 runtime-native agent/subagent 导入接口。Claude Code
subagents、OpenCode agent markdown、Cline rules 等对象与 AVM Agent 语义不等价，
不得通过 adapter 自动提升为 AVM Agent。
