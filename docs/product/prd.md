...
- `agent rename/delete` 会保护 active profile 和 env 引用。
...
- `status` 显示当前 active 对象、runtime 映射、managed paths、memory isolation
  status、warnings。
...
- `run <runtime>` 用于需要进程级隔离 envelope 的 runtime，例如 OpenCode。
...
- OpenCode 这类需要进程级变量的 runtime 通过 `avm run opencode` 注入
  `OPENCODE_DB` 和 `XDG_*`，不长期污染用户 shell。
